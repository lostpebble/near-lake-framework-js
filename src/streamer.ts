...
import { fetchStreamerMessage, listBlocks } from "./s3fetchers";
import { EPropertyNamingFormat, LakeConfig, StreamerMessage } from "./types";
...
async function* batchStream(
  ...
  if (config.propertyNamingFormat == null) {
    config.propertyNamingFormat = EPropertyNamingFormat.javascript;
  }
  ...
  while (true) {
    ...
    yield blockHeights.map((blockHeight) =>
      fetchStreamerMessage(
        s3Client,
        config.s3BucketName,
        blockHeight,
        config.propertyNamingFormat!
      )
    );
...
export async function* stream(
  ...
  while (true) {
    try {
      ...
      console.log(
        "Retrying on error when fetching blocks",
        e,
        "Refetching the data from S3 in 200ms"
      );
      await sleep(200);
    }
  }
